  ...
  useEffect(() => {
    ...
    if (timeline && container) {
      const timeoutId = setTimeout(() => {
        ...
        const containerHeight = container.clientHeight - 90;
        timeline.setDimensions({ width: containerWidth, height: containerHeight });
        setCanvasSize({ width: containerWidth, height: containerHeight });
        timeline.requestRenderAll();
      }, 350);
      return () => clearTimeout(timeoutId);
    }
  }, [isControlPanelCollapsed, timeline]);

  useEffect(() => {
    const canvasEl = canvasElRef.current;
    const scrollbarEl = horizontalScrollbarVpRef.current;
    
    if (!canvasEl || !scrollbarEl) return;
    ...
    if (playHeadPos >= canvasBoudingX) {
      const scrollDivWidth = scrollbarEl.clientWidth;
      const totalScrollWidth = scrollbarEl.scrollWidth;
      const currentPosScroll = scrollbarEl.scrollLeft;
      const availableScroll = totalScrollWidth - (scrollDivWidth + currentPosScroll);
      const scaleScroll = availableScroll / scrollDivWidth;
      
      if (scaleScroll >= 0) {
        if (scaleScroll > 1) {
          scrollbarEl.scrollTo({ left: currentPosScroll + scrollDivWidth });
        } else {
          scrollbarEl.scrollTo({ left: totalScrollWidth - scrollDivWidth });
        }
      }
    }
  }, [currentFrame]);
