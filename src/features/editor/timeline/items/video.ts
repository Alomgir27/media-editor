...
class Video extends VideoBase {
  ...
  private async loadFallbackThumbnail() {
    ...
    if (!fallbackThumbnail) {
      ...
      this.createDefaultVideoThumbnail();
      return;
    }
    ...
    return new Promise<void>((resolve) => {
      ...
      img.onerror = () => {
        // If image fails, try to generate thumbnail from video
        this.generateVideoThumbnail(fallbackThumbnail).then(() => {
          resolve();
        }).catch(() => {
          this.createDefaultVideoThumbnail();
          resolve();
  ...
  private createDefaultVideoThumbnail() {
    const canvas = document.createElement("canvas");
    const ctx = canvas.getContext("2d")!;
    
    // Default aspect ratio
    const targetHeight = 40;
    const targetWidth = Math.round(targetHeight * 16/9); // 16:9 aspect ratio
    ...
    ctx.fillText('▶', targetWidth/2, targetHeight/2 + 6);
  ...
  private async generateVideoThumbnail(videoSrc: string): Promise<void> {
  ...
  public drawTextIdentity(ctx: CanvasRenderingContext2D) {
    ...
    // Show filename if available, otherwise show "Video"
    const metadata = this.metadata as { filename?: string };
    const displayText = metadata?.filename || "Video";
    ctx.fillText(displayText, 36, 10);
