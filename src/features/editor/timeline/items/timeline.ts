...
class Timeline extends TimelineBase {
  ...
  public onScrollChange = throttle(async () => {
    ...
    this.requestId = requestAnimationFrame(() => {
      ...
      for (const object of objects) {
            ...
            if (this.isObjectInViewport(objectBounds, viewportBounds)) {
  ...
  private getViewportBounds() {
    ...
    return {
      left: -vt[4],
      top: -vt[5],
      right: -vt[4] + this.width,
      bottom: -vt[5] + this.height,
    };
  }

  private isObjectInViewport(objectBounds: { left: number; right: number; top: number; bottom: number }, viewportBounds: { left: number; right: number; top: number; bottom: number }) {
    return !(
      objectBounds.left > viewportBounds.right ||
      objectBounds.right < viewportBounds.left ||
      objectBounds.top > viewportBounds.bottom ||
      objectBounds.bottom < viewportBounds.top
    );
  }
