...
import { SquareSplitHorizontal, Trash, ZoomIn, ZoomOut, Mic, MicOff } from "lucide-react";
import {
  getFitZoomLevel,
  getNextZoomLevel,
...
import { AudioMode } from "../interfaces/layout";
...
const Header = () => {
  ...
  const handleAudioModeToggle = (mode: AudioMode) => {
    const newMode = mode === activeAudioMode ? (mode === "original" ? "english" : "original") : mode;
    setActiveAudioMode(newMode);
  };
          ...
          <div className="flex px-2">
            ...
            <div className="flex items-center gap-2 ml-4 pl-4 border-l border-border">
              <Button
                ...
                variant={activeAudioMode === "original" ? "secondary" : "ghost"}
                size={"sm"}
                className="flex items-center gap-1 px-2"
              >
                {activeAudioMode === "original" ? <Mic size={14} /> : <MicOff size={14} />} Original
              </Button>

              <Button
                onClick={() => handleAudioModeToggle("english")}
                variant={activeAudioMode === "english" ? "secondary" : "ghost"}
                size={"sm"}
                className="flex items-center gap-1 px-2"
              >
                {activeAudioMode === "english" ? <Mic size={14} /> : <MicOff size={14} />} English
              </Button>
              ...
              <div className="flex items-center gap-2 ml-2 pl-2 border-l border-border">
                <input
                  type="checkbox"
                  id="instruments"
                  checked={instrumentsEnabled}
                  onChange={(e) => setInstrumentsEnabled(e.target.checked)}
