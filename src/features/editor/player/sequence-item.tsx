...
interface SequenceItemOptions {
  ...
  activeAudioMode?: "original" | "english";
...
> = {
  ...
  video: (item, options: SequenceItemOptions) => {
    const { fps, zIndex, activeAudioMode = "english" } = options;
    ...
    const baseVolume = (details.volume !== undefined ? details.volume : 100) / 100;
    const videoVolume = activeAudioMode === "original" ? baseVolume : 0;
  ...
  audio: (item, options: SequenceItemOptions) => {
    const { fps, zIndex, activeAudioMode = "english", instrumentsEnabled = true } = options;
    ...
    const baseVolume = (details.volume || 100) / 100;
    let finalVolume = 0;

    if (item.metadata?.audioType === "instruments") {
      finalVolume = instrumentsEnabled ? baseVolume : 0;
    } else if (item.metadata?.englishText || item.metadata?.originalText) {
      finalVolume = activeAudioMode === "english" ? baseVolume : 0;
    } else {
      finalVolume = activeAudioMode === "original" ? baseVolume : 0;
    }
